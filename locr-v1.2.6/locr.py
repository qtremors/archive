...
def generate_json_report(
    ...
    """Generates a JSON string of the results."""
    ...
    # Format language stats (strip out color objects for JSON)
...
def main():
    epilog_text = (
        "Examples:\n"
        "  locr                      # Scan current directory\n"
        "  locr src --stats          # Scan 'src' with detailed stats\n"
        "  locr --raw                # Scan ignoring .gitignore\n"
        "  locr -o report.txt        # Save report to file\n"
        "  locr --json               # Output JSON to stdout"
    )
    ...
    if is_writing_file:
        filename = auto_out_name(target_path, is_json=args.json) if args.out is True else args.out
        try:
            content = ""
            if args.json:
                content = generate_json_report(results, end_time - start_time, engine.was_interrupted)
            else:
                # Re-generate clean table for file
        ...
        if args.json:
            print(generate_json_report(results, end_time - start_time, engine.was_interrupted))
        else:
            print("\n".join(report_lines))
